...
destination=${2:-dist/reference-service}
...
if [ "$(uname -s)" != Linux ]; then
    echo "reference-service deployment artifacts must be built on Linux" >&2
    exit 2
fi
...
install -m 0755 target/release/kult-reference-service \
    "$artifact_dir/kult-reference-service"
sha256sum "$artifact_dir/kult-reference-service" \
    > "$artifact_dir/SHA256SUMS"

rustc_version=$(rustc --version)
cat > "$artifact_dir/provenance.json" <<EOF
{
  "schema": "komms-reference-artifact-provenance/1",
...
tar --sort=name \
    ...
    -C "$artifact_dir" \
    -cf "$artifact_dir.tar" \
    SHA256SUMS kult-reference-service provenance.json
