...
| **Messaging features** | Pairwise and sender-key group text, note-to-self, scheduled text, attachments, recorded audio, still-image editing, group mentions, and B9 safe text formatting are shipped through the shared APIs and all three shells. Formatting keeps exact source on wire/storage, derives a bounded inert local model, and never enables HTML, links, remote media, or network fetches. Delivery state remains the honest `queued → sent → delivered` ladder. |
