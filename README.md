...
| **Messaging features** | Pairwise and sender-key group text, note-to-self, scheduled text, attachments, recorded audio, still-image editing, and group mentions are shipped through the shared APIs and all three shells. Delivery state remains the honest `queued → sent → delivered` ladder. |
| **Private local organization** | B10 folders and B18 contact/conversation labels are shipped across storage, node, RPC/CLI, UniFFI, desktop, Android, and iOS. They remain sealed local metadata, compose deterministically, create zero transport work, and are preserved by `KKR4`. Message labels and conversation pins remain separate work. |
...
`KKR4`. The principal release gaps are the physical radio bench, hands-on mobile
