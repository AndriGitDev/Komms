...
- The poll creator alone closes it. Closure freezes the visible vote snapshot
  the creator has received and cannot be undone.
...
- Polls survive restart and encrypted `KKR5` backup/restore as ordinary sealed
...
The creator’s close event carries the sorted heads it accepted. If conflicting
valid creator closures ever arrive, the smallest close event ID wins. The tally
is always derived locally from these authenticated immutable events, never from
a server or mutable counter.
...
`group_poll_vote`, and `group_poll_close`. The CLI mirrors them as
`group-poll-create`, `group-polls`, `group-poll-vote`, and
`group-poll-close`. Poll, option, author, voter, group, and event identifiers
are explicit; RPC rejects unknown and ambiguous fields.

UniFFI exposes the same create/list/vote/close calls, `GroupPoll`, `PollOption`,
...
show creator-only closure. Shells refresh from the node snapshot on events;
...
host-mobile bindings, and exact KKR5 restore. Android APK/device qualification
