...
| **Private local organization** | B10 folders, B11 conversation pins, and B18 contact/conversation labels are shipped across storage, node, RPC/CLI, UniFFI, desktop, Android, and iOS. They remain sealed local metadata, compose as folder → labels → pins/activity, create zero transport work, and are preserved by `KKR6`. Message pins and message labels remain separate work. |
...
Older `KKR1` through `KKR5` backups remain restorable; current backups are
`KKR6`. KKR6 carries signed group authority state and consumed admin-request
ids, excludes live ephemeral plaintext/media, and carries terminal tombstones
so restore cannot resurrect removed content. The principal release gaps are the physical radio bench, hands-on mobile
