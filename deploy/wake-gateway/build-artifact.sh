...
destination=${2:-dist/wake-gateway}
...
if [ "$(uname -s)" != Linux ]; then
    echo "wake-gateway deployment artifacts must be built on Linux" >&2
    exit 2
fi
...
cargo build --locked --release --package kult-wake
mkdir -p "$artifact_dir"
install -m 0755 target/release/kult-wake "$artifact_dir/kult-wake"
sha256sum "$artifact_dir/kult-wake" > "$artifact_dir/SHA256SUMS"

rustc_version=$(rustc --version)
cat > "$artifact_dir/provenance.json" <<EOF
{
  "schema": "komms-wake-artifact-provenance/1",
...
tar --sort=name \
    ...
    -C "$artifact_dir" \
    -cf "$artifact_dir.tar" \
    SHA256SUMS kult-wake provenance.json
