...
destination=${2:-dist/ohttp-relay}
...
if [ "$(uname -s)" != Linux ]; then
    echo "OHTTP-relay deployment artifacts must be built on Linux" >&2
    exit 2
fi
...
install -m 0755 target/release/kult-ohttp-relay "$artifact_dir/kult-ohttp-relay"
sha256sum "$artifact_dir/kult-ohttp-relay" > "$artifact_dir/SHA256SUMS"

rustc_version=$(rustc --version)
cat > "$artifact_dir/provenance.json" <<EOF
{
  "schema": "komms-ohttp-relay-artifact-provenance/1",
...
tar --sort=name \
    ...
    -C "$artifact_dir" \
    -cf "$artifact_dir.tar" \
    SHA256SUMS kult-ohttp-relay provenance.json
