//! Decode and verification support for legacy C2 account-rooted device state.
//!
//! Released Alpha stores and recovery fixtures still need this manifest shape
//! for the explicit ADR-0026 migration/reset boundary. Live authority and
//! routine device linking use `device_authority` and `device_link_authority`;
//! the legacy root-carrying link package is not compiled into production.
...
// The released C2 link package copied the stable account root. Keep its codec
// only for regression coverage of the rejected legacy design; no production
// API can create or open that package after ADR-0026.
#[cfg(test)]
#[allow(dead_code)]
mod legacy_device_link {
    use super::*;
    ...
    impl PendingDeviceLinkTarget {
        ...
        pub fn complete(
            ...
            let plain = Zeroizing::new(
                StorageKey::from_bytes(*self.link_key).open(LINK_PACKAGE_AD, package)?,
            );
            ...
            if account.public() != self.offer.manifest.account
                ...
                || !payload.manifest.devices.iter().any(|entry| {
                    entry.certificate == payload.certificate && entry.revoked_at.is_none()
                })
            {
                return Err(CryptoError::InvalidMessage);
            }
    ...
    pub fn seal_device_link_recovery_package(
        ...
        let certificate = manifest
            ...
            .iter()
            .find(|entry| {
                entry.certificate.device_id() == *target_device && entry.revoked_at.is_none()
            })
        ...
        let plain = Zeroizing::new(
            postcard::to_allocvec(&payload).map_err(|_| CryptoError::Serialization)?,
        );
...
mod tests {
    ...
    fn link_rejects_expiry_cancellation_and_tampering() {
        ...
        .is_err());
        let (source_pending, active_offer) =
            PendingDeviceLinkSource::begin(&account, &manifest, source.public().ed, 100, &mut rng)
                .unwrap();
        let (_, response, _) =
            PendingDeviceLinkTarget::accept(active_offer, &target, "Laptop".into(), 20, &mut rng)
                .unwrap();
