...
|---|---|---|
| Windows 10/11 x64 | `Komms-0.3.0-windows-x64.msi` or `Komms-0.3.0-windows-x64-setup.exe` | Pick one installer format; both install the same Alpha. |
| macOS Intel or Apple silicon | `Komms-0.3.0-darwin-universal.dmg` | Universal application. The `.app.tar.gz` is also available for manual deployment. |
| Linux x86-64 | `Komms-0.3.0-linux-amd64.AppImage`, `.deb`, or `.rpm` | Use the format native to your distribution. |
| Android 8.0+ | `Komms-0.3.0-android-debug.apk` | Test-only, debug-signed APK for `arm64-v8a` phones and `x86_64` emulators. |
...
```sh
shasum -a 256 Komms-0.3.0-darwin-universal.dmg
grep 'Komms-0.3.0-darwin-universal.dmg' SHA256SUMS
```

On Windows PowerShell:

```powershell
Get-FileHash .\Komms-0.3.0-windows-x64.msi -Algorithm SHA256
Select-String -Path .\SHA256SUMS -Pattern 'Komms-0.3.0-windows-x64.msi'
```
...
```sh
chmod +x Komms-0.3.0-linux-amd64.AppImage
./Komms-0.3.0-linux-amd64.AppImage
```

On Debian or Ubuntu:

```sh
sudo apt install ./Komms-0.3.0-linux-amd64.deb
```

On Fedora or another RPM-based distribution:

```sh
sudo dnf install ./Komms-0.3.0-linux-x86_64.rpm
```
...
1. Create and unlock a fresh identity on each device with a strong, unique
   ...
   issue report.
2. Exchange the pairing bundle or collect the complete animated QR sequence
   over a channel you trust.
3. Compare the complete 30-digit safety number through a separate trusted
   channel, or scan the full-value safety QR, before treating the contact as
   verified.
...
volunteering mailbox or bridge capacity. `0.3.0` is the immutable release tag;
`0.3-alpha` and `alpha` are moving Alpha aliases. There is intentionally no
`latest` tag.
